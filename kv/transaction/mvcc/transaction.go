...
// 完成基于 userKey、 lock、 write、 read 操作
type MvccTxn struct {
	StartTS uint64
...
func (txn *MvccTxn) GetValue(key []byte) ([]byte, error) {
	// Your Code Here (4A).
	// 1. 先在 CfWrite 找到 write，从而获取 write 的时戳
	// 2. 再在 CfDefault 中根据 key 和 timeS 来寻找数据
	...
	item := iter.Item()
	userKey := item.Key()
	if !bytes.Equal(key, DecodeUserKey(userKey)) {
		return nil, nil
	}
	i_value, err := item.Value()
	if err != nil {
		return nil, err
	}
	write, err := ParseWrite(i_value)
	if err != nil {
		return nil, err
	}
...
func (txn *MvccTxn) CurrentWrite(key []byte) (*Write, uint64, error) {
	...
	for iter.Seek(EncodeKey(key, ^uint64(0))); iter.Valid(); iter.Next() {
		...
		if write.StartTS == txn.StartTS {
			return write, decodeTimestamp(userKey), nil
		}
	}
	return nil, 0, nil
}
